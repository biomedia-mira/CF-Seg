...
padchest_var_to_df = {"sex":"sex_label",
                      "age":"age",
                      "view":"ViewPosition_label",
                      "race":"race_label",
                      "finding":"disease_label",
...
class PadChestDataGenerator(Dataset):
    ...
    def __init__(self, 
        ...
        self.samples = {}
        for i in parents_x:
            # print(f"{i}: {padchest_var_to_df[i]}")
            sample = torch.tensor(self.data[padchest_var_to_df[i]])            
            if i == "age":
                self.samples[i] = sample.float() / 100 * 2 -1 
            else:
                n_classes = padchest_var_shape[i]
                if i in dummy_var and add_dummy_dim:
                    n_classes += 1
                self.samples[i] = F.one_hot(sample,num_classes=n_classes).squeeze()
    ...
    def __getitem__(self, idx):
        ...
        dicom_id = self.data.iloc[idx]["ImageID"]

        # Generate data
        X = cv2.imread(os.path.join(self.inputpath,dicom_id)) # (256,256,3)
        y = cv2.imread(os.path.join(self.labelpath,dicom_id)) # (256,256,3) # left lung, right lung, heart
        ...
        for k in self.parents_x:
            ...
            if k == "age":
                ...
                num_classes = padchest_var_shape[k]
                if k in self.dummy_var and self.add_dummy_dim:
                    num_classes += 1 # extra dimension for gumble softmax and softmax_centered transform
                sample[k] = F.one_hot(sample[k], num_classes=num_classes).squeeze()
...
def padchest(args: Hparams, augment=True) -> Dict[str, CheXMaskDataGenerator]:

    csv_dict = {"train":args.train_csv, "test":args.test_csv, "valid":args.valid_csv}
    datasets = {}
    for split in ["train", "valid", "test"]:
        datasets[split] = PadChestDataGenerator(
                 inputpath = args.inputpath,
                 labelpath = args.labelpath,
                 augment = True if (split=="train" and augment==True) else False,
                 scale_range = args.scale_range, 
