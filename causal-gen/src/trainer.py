...
from plotting_utils import write_images


def preprocess_batch(args: Hparams, batch: Dict[str, Tensor], expand_pa: bool = False):
    # batch["x"] = batch["x"].to(args.device).float() / 255  # [0, 1]
    batch["x"] = batch["x"].to(args.device).float()
...
def trainer(args: Hparams, model: nn.Module, ema: nn.Module, dataloaders: Dict[str, DataLoader], optimizer: torch.optim.Optimizer, scheduler: Any, writer: SummaryWriter, logger: logging.Logger,):
    ...
    def run_epoch(dataloader: DataLoader, training: bool = True):
        ...
        loader = tqdm(enumerate(dataloader), total=len(dataloader), mininterval=mininterval)
        ...
        for i, batch in loader:
            ...
            if training:
                ...
                if args.beta_warmup_steps > 0:
                    args.beta = args.beta_target * linear_warmup(args.beta_warmup_steps)(args.iter)
                ...
                if i % args.accu_steps == 0:  # gradient accumulation update
                    ...
                    if grad_norm < args.grad_skip and nll_nan == 0 and kl_nan == 0:
                        ...
                        updates_skipped += 1
                        update_stats = False
                        logger.info(f"Updates skipped: {updates_skipped} - grad_norm: {grad_norm:.3f} - nll_nan: {nll_nan.item()} - kl_nan: {kl_nan.item()}")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        logger.info(f'=> train | nelbo: {stats["elbo"]:.4f} - nll: {stats["nll"]:.4f} - kl: {stats["kl"]:.4f} - steps: {args.iter}')
        ...
        if (args.epoch - 1) % args.eval_freq == 0:
            ...
            logger.info(f'=> valid | nelbo: {valid_stats["elbo"]:.4f} - nll: {valid_stats["nll"]:.4f} - kl: {valid_stats["kl"]:.4f} - steps: {args.iter}')

            if valid_stats["elbo"] < args.best_loss:
                args.best_loss = valid_stats["elbo"]
                ckpt_path = os.path.join(args.save_dir, "checkpoint.pt")
                torch.save(save_dict, ckpt_path)
                logger.info(f"New Best Model saved: {ckpt_path} at Epoch {args.epoch}")
