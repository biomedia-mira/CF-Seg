...
def gaussian_kl(q_loc, q_logscale, p_loc, p_logscale):
    ...
    out = dist.kl_divergence(dist.Normal(q_loc, q_scale), dist.Normal(p_loc, p_scale),)
    return out
...
class Block(nn.Module):
    def __init__(self, args, in_width, out_width, kernel_size=3, residual=True, down_rate=None, condition=True,):
        ...
        if self.condition:
            self.pa_proj = nn.Sequential(activation, nn.Linear(args.embd_dim, bottleneck))
    ...
    def __init__(self, args):
        ...
        for i, stage in enumerate(args.enc_arch.split(",")):
            ...
            if i == 0:  # define network stem
                ...
                self.stem = nn.Conv2d(args.input_channels, stem_width, kernel_size=7, stride=stem_stride, padding=3,)
...
class DecoderBlock(nn.Module):
    def __init__(self, args, in_width, out_width, z_dim, resolution):
        ...
        self.prior = Block(args, in_width, 2 * self.z_dim + in_width, kernel_size=k, residual=False, condition=self.cond_prior,)

        if self.stochastic:
            self.posterior = Block(args, 2 * in_width, 2 * self.z_dim, kernel_size=k, residual=False,)
...
class Decoder(nn.Module):
    def __init__(self, args):
        ...
        for i, (res, width, z_dim) in enumerate(stages):
            ...
            blocks.append(DecoderBlock(args, width, next_width, z_dim, res))
        self.blocks = nn.ModuleList(blocks)
        ...
        self.pa_embd = nn.Sequential(nn.Linear(args.context_dim, args.embd_dim), nn.SiLU(), nn.Linear(args.embd_dim, args.embd_dim),)
    ...
    def forward(self, parents, x=None, t=None, abduct=False, latents=[]):
        ...
        for i, block in enumerate(self.blocks):
            ...
            if block.q_correction:
                ...
                p_input = (b + F.interpolate(z, scale_factor=res / z.shape[-1]) if z.size(-1) < res else z)
                ...
                if x is not None:  # z_i ~ q(z_i | z_<i, x, pa_x)
                    ...
                    if abduct:  # abduct exogenous noise
                        if block.cond_prior:  # z* if conditional prior
                            stat.update(dict(z={"z": z, "q_loc": q_loc, "q_logscale": q_logscale}))
                        else:
                    ...
                    try:  # forward fixed latents z or z*
                        ...
                        if abduct and block.cond_prior:  # for abducting z*
                            stats.append(dict(z={"p_loc": p_loc, "p_logscale": p_logscale}))
            ...
            if not block.q_correction:
                if (i + 1) < len(self.blocks):  # z independent of pa_x for next layer prior
...
class DGaussNet(nn.Module):
    def __init__(self, args):
        ...
        self.x_loc = nn.Sequential(nn.GroupNorm(min(32, width // 4), width), nn.SiLU(), zero_module(nn.Conv2d(width, args.input_channels, 3, padding=1)),)

        self.x_logscale = nn.Sequential(nn.GroupNorm(min(32, width // 4), width), nn.SiLU(), zero_module(nn.Conv2d(width, args.input_channels, 3, padding=1)),)
    ...
    def forward(self, h, x=None, t=None):
        ...
        if self.covariance != "fixed":
            ...
            log_scale = (torch.ones_like(loc, requires_grad=False, device=loc.device)* self.logscale)
...
class HVAE(nn.Module):
    ...
    def forward(self, x, parents, beta=1):
        ...
        if self.free_bits > 0:
            ...
            kl_pp = 0.0
            for stat in stats:
                kl_pp += torch.maximum(free_bits, stat["kl"].sum(dim=(2, 3)).mean(dim=0)).sum()
        else:
    ...
    def abduct(self, x, parents, cf_parents=None, t=None, alpha=0.8):
        acts = self.encoder(x)
        _, q_stats = self.decoder(x=acts, parents=parents, abduct=True, t=t)  # q(z|x,pa)
