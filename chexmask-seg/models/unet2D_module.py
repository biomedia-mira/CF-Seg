...
class Unet2D(nn.Module):
    def __init__(self,
        ...
        if self.last_layer == "aleatoric":
            ...
            self.final = nn.Conv2d(in_channels = self.init_ker, 
                                   out_channels = self.out_chan, 
                                   kernel_size = 1, 
                                   stride = 1, 
                                   bias = True)   
            # to map to desired output channel size - var
            self.final_var = nn.Conv2d(in_channels = self.init_ker, 
                                   out_channels = self.out_chan, 
                                   kernel_size = 1, 
                                   stride = 1, 
                                   bias = True)   
...
class convBlocks(nn.Module):
    def __init__(self,
        ...
        self.model += [conv2dBlock(inp_chan = inp_chan, 
                                   out_chan = out_chan, 
                                   kern_size = kern_size,
                                   stride = 1, 
                                   padding = 1, 
        ...
        for _ in range(n_layers-2):
            self.model += [conv2dBlock(inp_chan = out_chan, 
                                       out_chan = out_chan, 
                                       kern_size = kern_size, 
                                       stride = 1, 
                                       padding = 1, 
...
class convTransBlocks(nn.Module):
    def __init__(self,
        ...
        if us_type == 'transpose':
            self.upsamp = convTranspose2dBlock(inp_chan = inp_chan, 
                                               out_chan = out_chan, 
                                               kern_size = 3, 
                                               stride = 2, 
                                               padding = 1, 
            ...
            m += [nn.Upsample(scale_factor = 2, 
                              mode = 'nearest')]
            m += [conv2dBlock(inp_chan = inp_chan, 
                              out_chan = out_chan, 
                              kern_size = 3, 
                              stride = 1, 
                              padding = 1, 
...
class conv2dBlock(nn.Module):
    def __init__(self,
        ...
        if pad_type == 'replicate':
            ...
            self.pad = nn.ConstantPad2d(padding=padding, value=0)
        else:
        ...
        self.conv = nn.Conv2d(in_channels = inp_chan, 
...
class convTranspose2dBlock(nn.Module):
    def __init__(self, 
        ...
        # initialize convolution
        self.convtp = nn.ConvTranspose2d(in_channels = inp_chan, 
                                         out_channels = out_chan, 
                                         kernel_size = kern_size, 
                                         stride = stride, 
                                         padding = padding, 
